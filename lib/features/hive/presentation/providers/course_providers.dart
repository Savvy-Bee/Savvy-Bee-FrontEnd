...
// FIX 3: initialize() was never called, so progress was always lost on restart.
// The provider factory now calls notifier.initialize() immediately, which loads
// the saved JSON from SharedPreferences and rebuilds state before any widget
// reads it. Because initialize() is async and state updates reactively, widgets
// will re-render automatically once the saved data is applied.
final quizProgressProvider =
    StateNotifierProvider<QuizProgressNotifier, QuizProgressTracker>((ref) {
      final repository = ref.watch(progressRepositoryProvider);
      final notifier = QuizProgressNotifier(repository);
      notifier.initialize(); // ← THE MISSING CALL
      return notifier;
    });
...
final courseStatsProvider = Provider.family<CourseStats, Course>((ref, course) {
  ...
  final int totalLevels = course.totalLevels;
