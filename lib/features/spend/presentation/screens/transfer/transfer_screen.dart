// ─────────────────────────────────────────────
// transfer_screen.dart  (replaces the old one)
// ─────────────────────────────────────────────
import 'dart:async';
...
class _TransferScreenState extends ConsumerState<TransferScreen> {
  ...
  bool get _canContinue {
    ...
    return transferState.verifiedAccount != null && !_isVerifying;
  }
  ...
  void _scheduleVerification() {
    _debounceTimer?.cancel();
    if (_accNumberController.text.length == 10 && _selectedBank != null) {
      _debounceTimer = Timer(const Duration(milliseconds: 500), _verifyAccount);
    }
  }

  // ── Continue button → show bottom sheet ─────────────────────────────────
  void _onContinue() {
    final verifiedAccount = ref.read(transferNotifierProvider).verifiedAccount;
  ...
  Widget build(BuildContext context) {
    ...
    final canContinue = transferState.verifiedAccount != null && !_isVerifying;

    return Scaffold(
      appBar: AppBar(
        title: const Text('Send to bank account'),
      ...
      body: ListView(
        ...
        children: [
          ...
          _VerificationStatus(
            ...
            verifiedName: transferState.verifiedAccount?.accountName,
          ),
          const Gap(16),
          ...
          banksAsync.when(
            data: (banks) {
              final nigerianBanks =
                  banks.where((b) => b.isNigerianBank).toList()
                    ..sort((a, b) => a.name.compareTo(b.name));

              return CustomDropdownButton(
                items: nigerianBanks.map((b) => b.name).toList(),
...
class _VerificationStatus extends StatelessWidget {
  ...
  const _VerificationStatus({required this.isVerifying, this.verifiedName});
...
// ── Account confirmation bottom sheet ────────────────────────────────────────
class _AccountConfirmationBottomSheet extends ConsumerWidget {
  ...
  static void show(
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _AccountConfirmationBottomSheet(
        accountName: accountName,
        accountNumber: accountNumber,
        bankName: bankName,
        bankCode: bankCode,
