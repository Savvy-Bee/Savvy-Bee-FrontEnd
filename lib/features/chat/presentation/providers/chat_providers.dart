...
// myPersonaProvider
//
// Calls GET /auth/update/getmypersona and returns the Persona model.
// The chat_screen watches this to resolve the matching local image + name.
// ─────────────────────────────────────────────────────────────────────────────

final myPersonaProvider = FutureProvider<Persona?>((ref) async {
  final repo = ref.read(chatRepositoryProvider);
  return repo.fetchMyPersona();
});

// ─────────────────────────────────────────────────────────────────────────────
// Legacy provider (kept so other screens that import it don't break)
// ─────────────────────────────────────────────────────────────────────────────

final aiPersonalityProvider = FutureProvider<List<Personality>>((ref) async {
  final repo = ref.read(chatRepositoryProvider);
  return repo.fetchPersonalities();
});
...
class ChatNotifier extends AsyncNotifier<ChatState> {
  ...
  Future<bool> sendMessage(
    ...
    try {
      ...
      if (response != null && response.success) {
        log('Message sent successfully. Received ${response.chats.length} messages');
        for (final chat in response.chats) {
          log('Received message: type=${chat.chatType}, hasWidget=${chat.hasWidget}');
        }

        final updatedMessages = [
          ...currentState.messages.where((msg) => msg.id != optimisticUserMessage.id),
          ...response.chats,
        ];
