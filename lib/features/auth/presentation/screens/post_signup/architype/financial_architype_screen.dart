...
class _FinancialArchitypeScreenState
  ...
  void _handleSkip() {
    // Show notification prompt when skipping
  ...
  Future<void> _submitOnboarding() async {
    ...
    try {
      ...
      // The user must be logged in to reach this screen (either from signup
      // or from resuming an incomplete registration).
      ...
      final request = PostOnboardRequest(
        ...
        financePriorities: ref.read(financialPriorityProvider)?.text ?? '',
        howFinanceManaged: ref.read(financeManagementProvider)?.text ?? '',
        confusingTopics: ref.read(confusingTopicProvider)?.text ?? '',
        challengesWithYou: ref.read(financialChallengeProvider)?.text ?? '',
        motivatesyou: ref.read(financialMotivationProvider)?.text ?? '',
      );
