...
class FinancialArchitypePageTwo extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final selected = ref.watch(financialPrioritiesProvider);
    ...
    return Column(
      ...
      children: FinancialPriority.values.map((priority) {
        ...
        return ToggleableListTile(
          ...
          onTap: () {
            final current = ref.read(financialPrioritiesProvider);
            ref.read(financialPrioritiesProvider.notifier).state = isSelected
                ? current.where((e) => e != priority).toList()
                : [...current, priority];
          },
        );
      }).toList(),
