...
/// Carries the email and the page index to resume from when a user
/// returns after an incomplete registration.
...
class _SignupScreenState extends ConsumerState<SignupScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Clamp the target page to a valid range.
      final targetPage = data.pageIndex.clamp(0, _dotCount - 1);

      // Jump without animation so the user lands directly on the right step.
      _pageController.jumpToPage(targetPage);
      setState(() => _currentPage = targetPage);

      // Pre-fill email so auto-OTP send and future API calls work correctly.
      if (data.email.isNotEmpty) {
        _emailController.text = data.email;
      }

      // Auto-send OTP when resuming on the email-verification page.
      if (targetPage == 4 && data.email.isNotEmpty) {
  ...
  void _goToPreviousPage() {
    ...
    // — there's nothing meaningful to go back to in that flow.
  ...
  String _getPageSubtitle() {
    ...
    // The OTP page (index 4) shows the destination email dynamically.
    if (_currentPage == 4) {
  ...
  /// Sends (or re-sends) the email OTP.
  /// [silent] skips the success snackbar — used for the automatic send on resume.
  ...
  Future<void> _handleContinue() async {
    ...
    try {
      switch (_currentPage) {
        // ── Page 0: First & last name ────────────────────────────────────────
        case 0:
          if (_nameFormKey.currentState!.validate()) _goToNextPage();
          break;

        // ── Page 1: Email ────────────────────────────────────────────────────
        case 1:
          if (_emailFormKey.currentState!.validate()) _goToNextPage();
          break;

        // ── Page 2: Username ─────────────────────────────────────────────────
        case 2:
          if (_usernameFormKey.currentState!.validate()) _goToNextPage();
          break;

        // ── Page 3: Password + register call ────────────────────────────────
        case 3:
      ...
      children: [
        CustomOtpField(
          onCompleted: (value) {
            ...
            // Auto-advance as soon as the last digit is entered.
            _handleContinue();
          },
        ),
        const Gap(20),
