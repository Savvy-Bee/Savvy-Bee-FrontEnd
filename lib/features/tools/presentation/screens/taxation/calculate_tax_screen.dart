...
import 'package:savvy_bee_mobile/features/tools/presentation/widgets/tax_reliefs_bottom_sheet.dart';
...
class _CalculateTaxScreenState extends ConsumerState<CalculateTaxScreen> {
          ...
          Row(
            ...
            children: [
              _buildMoreActionItem(
                ...
                'Export Tax\nSummary',
                onTap: () async {
                  final taxData = ref.read(taxCalculatorNotifierProvider).value;
                  if (taxData == null) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('No tax calculation to export'),
                  ...
                  try {
                    // Generate PDF
                    final filePath =
                        await PdfGeneratorUtil.generateTaxSummaryPdf(
                          taxData.data,
                        );
          ...
          Row(
            ...
            children: [
              ...
              _buildMoreActionItem(
                ...
                'Explain More\nTax Reliefs',
                onTap: () {
                  TaxReliefsBottomSheet.show(context);
                },
              ),
            ],
          ),
      ...
      child: Column(
        ...
        children: [
          ...
          _buildTaxBreakdownItem(
            'Gross Income',
            taxData.totalEarnings.toDouble().formatCurrency(decimalDigits: 0),
          ),
          const Gap(8),
          _buildTaxBreakdownItem(
            'Base Exemption',
            '-${(taxData.totalEarnings - taxData.tax.yearly).toDouble().formatCurrency(decimalDigits: 0)}',
          ),
          const Gap(8),
          _buildTaxBreakdownItem(
            'Taxable Income',
            taxData.tax.yearly.toDouble().formatCurrency(decimalDigits: 0),
          ),
          const Gap(8),
          _buildTaxBreakdownItem(
            'Tax Before Relief',
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            '≈ ${taxData.tax.monthly.toDouble().formatCurrency(decimalDigits: 0)}/month',
            style: TextStyle(color: AppColors.white),
          ),
          Container(
            padding: const EdgeInsets.symmetric(vertical: 20, horizontal: 10),
  ...
  Widget _buildUploadView() {
    final monthlyIncome = int.tryParse(monthlyIncomeController.text) ?? 0;
    final annualAmount = monthlyIncome * 12;
    ...
    return Column(
      children: [
        ...
        CustomTextFormField(
          ...
          hint: '₦800,000',
          subText: annualAmount > 0
              ? 'Annual: ${annualAmount.toDouble().formatCurrency(decimalDigits: 0)}'
              : null,
