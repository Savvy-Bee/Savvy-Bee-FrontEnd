...
import 'package:savvy_bee_mobile/features/tools/presentation/widgets/tax_filing/tax_ready_to_file_card.dart';
...
class _TaxationDashboardScreenState
  ...
  void _onRemindLater() {
    AppNotification.show(
      context,
      message: "We'll remind you in 24 hours to file your tax return.",
    );
  }

  // ── Navigate to filing screen ─────────────────────────────────────────────

  // void _onFileTap() => context.pushNamed(FilingRoutes.step1);
    // ── CHANGE: Taxpayer ID screen is now the entry point ─────────────────────
  void _onFileTap() => context.pushNamed(FilingRoutes.taxpayerId);
      ...
      child: Column(
        ...
        children: [
          ...
          ...taxHistory
              ...
              .map(
                (item) => Column(
                    children: [_buildTaxLeakItem(item), const Gap(10)]),
              ),
        ],
      ),
    );
      ...
      child: Row(
        ...
        children: [
          _buildQuickActionItem(
            ...
            label: 'Calculator',
            onTap: () => context.pushNamed(CalculateTaxScreen.path),
          ),
          _buildQuickActionItem(
            iconPath: AppIcons.barChartIcon,
            label: 'Tax Stats',
            onTap: () => context.pushNamed(TaxStatsScreen.path),
          ),
          _buildQuickActionItem(
          ...
          _buildQuickActionItem(
            ...
            label: 'Strategy',
            onTap: () => context.pushNamed(StrategyCenterScreen.path),
          ),
        ],
      ),
    );
